...
from FlaskWebProject.forms import LoginForm, PostForm, DeletePostForm
...
def delete_post(id):
    post = Post.query.get(int(id))
    form = DeletePostForm(formdata=request.form, obj=post)
    if form.validate_on_submit():
        post.delete_post(form, current_user.id)
        return redirect(url_for('home'))
    return render_template(
        'post.html',
        title='Delete Post',
